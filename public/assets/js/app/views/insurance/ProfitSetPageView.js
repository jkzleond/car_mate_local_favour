...
define([
    ...
    'text!templates/insurance/profit_set_page.html'
    ...
    var FullSetPageView = Backbone.View.extend({
        ...
        _onCarTypeIdChange: function(event){
            var car_type_id = $(event.currentTarget).val();
            if(car_type_id == '2')
            {
                this.$el.find('.truck-level').show();
                this.$el.find('.psg-car-level').hide();
            }
            else if(car_type_id == '4')
            {
                this.$el.find('.psg-car-level').show();
