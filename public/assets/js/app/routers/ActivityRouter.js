...
define([
    ...
    var ActivityRouter = Backbone.Router.extend({
        ...
        _onNewOrderGen: function(view, order_info)
        {
            ...
            */
            var order_info_xml = '<root><orderId>' + order_info.order_id + '<orderId><orderNo>' + order_info.order_no + '</orderNo><orderFee>' + order_info.order_fee + '</orderFee><payType><offline>' + order_info.pay_type.offline + '</offline><alipay>' + order_info.pay_type.alipay + '</alipay><wxpay>' + order_info.pay_type.wxpay + '</wxpay></payType><des>des</des></root>'; // 因为中文备注引起IOS订单支付不了的问题,暂时去了 <des> + order_info.order_des + </des>
